...
# Features INEC que necesita el modelo enriquecido
INEC_COLS = [
...
# Cargar dataset enriquecido para features INEC
...
# Lookup: features promedio por parroquia+canton (originales + INEC)
...
# Pre-calcular features INEC promedio por parroquia (para el formulario)
inec_lookup = {}
for key, vals in parroquia_features.items():
    inec_lookup[key[0]] = vals[len(FEATURE_COLS):]  # solo las INEC

# INEC promedio global (fallback si no se selecciona parroquia)
...
def predecir_riesgo(nombre, canton="", inec_vals=None):
    ...
    if key in parroquia_features:
        ...
        # Fallback: usar promedio features originales + INEC del nombre
        orig = np.array([
            df[df["PARROQUIA"].str.strip().str.title() == nombre.strip().title()][c].mean()
            if nombre.strip().title() in df["PARROQUIA"].str.strip().str.title().values
            else df[c].mean()
            for c in FEATURE_COLS
        ])
        if inec_vals is None:
            inec_vals = inec_lookup.get(nombre.strip().title(), inec_global_mean)
